...
public class ImageCodeGenerator implements ValidateCodeGenerator{
    ...
    public ImageCode createCode(ServletWebRequest request) {
        ...
        int width = ServletRequestUtils.getIntParameter(request.getRequest(),"width",browserProperties.getImage().getWidth());
        //高度（宽度）
        int height = ServletRequestUtils.getIntParameter(request.getRequest(),"height",browserProperties.getImage().getHeight());
        //图片验证码字符个数
        int length = browserProperties.getImage().getLength();
        //过期时间
        int expireIn = browserProperties.getImage().getExpireIn();
